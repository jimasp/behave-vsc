...
import {
  buildExpectedFriendlyCmdOrderedIncludes, checkExtensionIsReady, getRunProfile,
  getTestProjectUri, replaceBehaveIni, setCleanup
} from "./helpers";
...
function assertExpectedFriendlyCmdsForParallel(request: vscode.TestRunRequest, folderItem: FolderItemWithDescedents, expectedResults: TestResult[],
  ...
  expectedResultsFeaturesInFolder.forEach(expectedResult => {
    ...
    const expectCmdOrderedIncludes = buildExpectedFriendlyCmdOrderedIncludes(projUri, testExtConfig, runOptions, request, projName, [queueItem]);
