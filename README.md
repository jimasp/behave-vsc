...
- Step navigation limitations:
  - "Go to Step Definition" and "Find All Step References" are simple regex implementations, they are not type-aware, e.g. they do not distinguish between `{param}` and `{param:d}` and `{param:f}`.
  - "Find All Step References" will return mismatches where the step text ends with `{param}`. A warning will appear in the Step References window when this happens. (You can avoid mismatches by quoting `"{param}"` or adding a distinguishing string after/before the parameter e.g. `{param} thing`.)
  - "Go to Step Definition" context menu doesn't always match as per behave (and never will). This is because there are a lot of ways to specify step matching and parameters in behave - `parse`;`re`;`cfparse`, and we would have to recreate these matching algorithms exactly.
  - "Go to Step Definition" context menu will only find steps that are in `.py` files in a folder called `steps` that is in your features folder (e.g. if you import steps in python from a steps library folder outside your steps folder it won't find them).
