...
- *How can I only execute specific tags while using the extension?* There are a lot of options here, but there are four examples below. The last two options will not avoid the overhead associated with starting a behave process for each test, i.e. the last two will only be notably faster when either (a) you have `runAllAsOne` enabled (the default) and you run are running all tests in the workspace at once, or (b) you are skipping slow tests.
  ...
  - (fast, simple) use the `fastSkipTags` extension setting. (This is only for the inverse case of do *not* run these tags.)  
