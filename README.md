...
- *How does step keyword matching working when using Go to Step Definition or Find All Step References?* Step keyword matching should work as per behave, i.e. Go to Step Definition matches the keyword like this: "Given" -> @given/@step, "When" -> @when/@step, "Then" -> @then/@step, "And" or "But" -> their preceding (non-"And" or "But") keyword in the scenario/@step. Find All Step References basically does the reverse, it matches @given/@when/@then -> the keyword + "And" + "But", and @step -> everything. Note that although keyword matching should work as behave, step text matching itself *does not* (see [Known issues](#known-issues-and-limitations) below).
